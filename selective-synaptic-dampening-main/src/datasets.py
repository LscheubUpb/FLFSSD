'Entries: 14/50 (Not Modified Often)'
...
from torchvision.datasets import CIFAR100, CIFAR10, ImageFolder, CelebA
import torch
...
class PinsFaceRecognition(ImageFolder):
    image_paths = []
    def __init__(self, root, train, unlearning, download, img_size=32):
        # Choose the appropriate transform based on the mode
        if train:
            if unlearning:
                transform = transform_unlearning
            else:
        ...
        # Initialize the ImageFolder with the given transform
        super().__init__(root, transform)
        
        # Store the class-to-index mapping for easy access
        self.class_to_idx = self.find_classes(root)[1]
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        # No change to the return signature
        return x, image_path, y
    
class CelebADataset(CelebA):
    def __init__(self, root, train=True, unlearning=False, download=True, img_size=224):
        if train:
            if unlearning:
                transform = transform_unlearning
            else:
        ...
        # # Load the identity mappings
        # identity_file = os.path.join(root, 'identity_CelebA.txt')
        # partition_file = os.path.join(root, 'list_eval_partition.txt')
        
        # self.identity_df = pd.read_csv(identity_file, sep='\s+', header=None, names=['filename', 'identity'])
        # self.partition_df = pd.read_csv(partition_file, sep='\s+', header=None, names=['filename', 'partition'])

        # # Merge dataframes based on the filename
        # self.metadata = self.identity_df.merge(self.partition_df, on='filename')

        # Call the parent constructor
        super().__init__(root=root, split='train' if train else 'valid', download=False, transform=transform, target_type = "identity")
