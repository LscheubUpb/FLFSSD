...
import forget_class_extractor_strategies_individual
...
def create_unlearning_image_list(ds):
    fileName = f"./SimilarityLists/unlearned_single_part_img.list"
    i = 0
    with open(fileName, "w") as f:
        for _, path, _ in ds:
            i += 1
            f.write(f"C:/Users/leosc/Documents/_wichtigeDokumente/Bachelorarbeit/selective-synaptic-dampening-main/src/{path[1:]}\n")
    print(f"This dataset part has {i} images")


def split_dataset_into_single_classes(ds, root, num_classes):
    dataset_Parts = []
    previous_last_index = -1
    last_index = 0
    files = os.listdir(root)
    foldersInOrder = sorted(files, key=str)[:2*num_classes]
    for i in range(num_classes):
        target_folder_path = os.path.join(root, str(foldersInOrder[i]))
        last_index = previous_last_index + num_items_in_folder(target_folder_path)
        if(previous_last_index == -1):
            last_index += 1
        indices = range(previous_last_index+1, last_index)
        print(f"range {indices}")
        subset = Subset(ds, indices)
        dataset_Parts.append(subset)
        previous_last_index = last_index
    
    indices_second_group = range(last_index + 1, 5822653)

    subset2 = Subset(ds, indices_second_group)
    del ds

    # For rearranging the classes (1,2,3,4,5 -> 3,1,2,4,5)
    tmp = dataset_Parts.copy()
    # dataset_Parts[0] = tmp[2]
    # dataset_Parts[1] = tmp[0]
    # dataset_Parts[2] = tmp[1]

    # Skipping class 3
    # dataset_Parts[2] = tmp[3]
    # dataset_Parts[3] = tmp[4]
    # dataset_Parts[4] = tmp[5]

    return dataset_Parts, subset2
...
if __name__ == '__main__':
    ...
    parser.add_argument("-weight_path",type=str, default="MagFace", help="The weights/checkpoint path")# choices=["MagFace","ArcFace","<Path>"]
    ...
    parser.add_argument("-dataset",type=str,default="agedb",nargs="?",choices=["agedb", "lfw", "cfp", "Adience","Pins"],help="dataset to test on",)
    ...
    parser.add_argument('-forgetClasses', type=int ,default=10, help='Number of classes to forget')
    ...
    if(args.forgetImages):
        args.forgetClasses = 0

    forget_trainsets, retain_train = split_dataset_into_single_classes(trainset, root, args.forgetClasses)

    part = 0
    for forget_train in forget_trainsets:
        os.environ['PART'] = str(part)
        create_unlearning_image_list(forget_train)
        ...
        last_checkpointPath = os.getenv('last_checkpoint',"None")
        if(last_checkpointPath != "None"):
            args.weight_path = last_checkpointPath
            args.resume = last_checkpointPath
        ...
        if not args.noSSD:
            for dampening_constant in dampening_constants:
                ...
                os.environ['fileName'] = f"{args.arch}_{args.dataset}_{args.ssd}_individual_{part}of{args.forgetClasses}_lambda{dampening_constant}_alpha{selection_weighting}"
                ...
                for selection_weighting in selection_weightings:
                    ...
                    os.environ["ident"] = f"{args.arch}_{args.dataset}_{args.ssd}_individual_{part}of{args.forgetClasses}_lambda_{dampening_constant}_alpha_{selection_weighting}"
                    
                    print(Fore.GREEN + f"Running Run Alpha {selection_weighting}, Lambda {dampening_constant}" + Style.RESET_ALL)
                    start = time.time()
                    mean_acc, std_acc, tpr, fpr, cosSim_Unlearned, cosSim_Reference, mia, D_f_r, mia_method = getattr(forget_class_extractor_strategies_individual, args.method)(**kwargs)
                    end = time.time()
                    ...
                    with open(aggregateResultsFile, "a") as f:
                        f.write(f"{args.arch}_{args.dataset}_{args.ssd}_individual_{part+1}/{args.forgetClasses}_lambda:{dampening_constant}_alpha:{selection_weighting}_{mia_method}: {D_f_r}, {cosSim_Unlearned}, {cosSim_Reference}, {int(time_elapsed)}, {std_acc}, {mean_acc}\n")
