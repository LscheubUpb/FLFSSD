'Entries: 2/50 (Not Modified Often)'
...
parser.add_argument('--offset', type=int,
                    default=0,
                    help='Set offset to 1 for CelebA')                    
...
def load_feat_pair(feat_path, pair_path, offset = 0):
    pairs = {}
    with open(feat_path) as f:
        ls = f.readlines()
    with open(pair_path, 'r') as f:
        pairs_info = [line.strip().split() for line in f.readlines()]
        for idx in range(len(pairs_info)):
...
def perform_1v1_eval(args):
    ...
    feat_pairs = load_feat_pair(feat_list, pair_list, args.offset)
    # ensemble feats
    embeddings0 = []
    embeddings1 = []
    targets = []

    for k, v in tqdm(feat_pairs.items(), desc="Evaluating Pairwise accuracy"):
        feat_a = v[0]
        feat_b = v[1]
        ab_is_same = int(v[2])
        ...
        try:
            ...
            # Path has a space case
            np_feat_a = np.asarray(feat_a.split()[2:513], dtype=float)
            np_feat_b = np.asarray(feat_b.split()[2:513], dtype=float)
        # append
