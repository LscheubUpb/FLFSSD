'Entries: 9/50 (Not Modified Often)'
...
class ImgInfLoader(data.Dataset):
    def __init__(self, ann_file, transform=None):
        self.PathPrefix = os.getenv('PATH_PREFIX',"") #"C:/Users/leosc/Documents/_wichtigeDokumente/Bachelorarbeit/MagFace/eval/eval_recognition/" #""
    ...
    def __getitem__(self, index):
        ...
        if img is None:
            ...
            exit(1)
        _img = cv2.flip(img, 1)
        return [self.transform(img), self.transform(_img)], img_path # type: ignore
...
def main_worker(ngpus_per_node, args):
    ...
    with torch.no_grad():
        ...
        for i, (input, img_paths) in tqdm(enumerate(inf_loader), total=len(inf_loader)):
